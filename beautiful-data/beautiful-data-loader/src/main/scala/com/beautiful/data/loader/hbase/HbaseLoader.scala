...
object HbaseLoader {
  ...
  def read(session:SparkSession, conf:Map[String,String]):Dataset[Row]= {
    ...
    session.sparkContext.hadoopConfiguration.set("hbase.zookeeper.property.clientPort", conf.get("hbase.zookeeper.property.clientPort") match {case None=>"2181" case _ =>conf.get("hbase.zookeeper.property.clientPort").get})
